...
public class Constants {
  ...
  public static boolean iTRAQSearch = false;
  ...
  public static boolean isInModifiedRange(double v) {
    if (minModifiedMass - gapTolerance < v
     && v < maxModifiedMass + gapTolerance)
      return true;
    else if (Math.abs(v) <= gapTolerance)
      return true;
    else
      return false;
  }
  ...
  public static int[] maxPTMOccurrence = {1, 1, 2, 2, 3, 3, 3, 2};
  // = new int[7];

  public static int getMaxPTMOccurrence(int seqLength) {
    if (seqLength > 10 || seqLength < 2)
      return 1;
    else
      return 2;
  }
  ...
  public static boolean fEqual(double v1, double v2) {
    if (Math.abs(v1 - v2) <= fragmentTolerance)
      return true;
    else
      return false;
  }
  ...
  public static boolean pEqual(double v1, double v2) {
    if (Math.abs(v1 - v2) <= precursorTolerance)
      return true;
    else
      return false;
  }
  ...
  public static final double[]
      rNorm =
      {6, 2.928968, 1.928968, 1.428968, 1.095635, 0.845635, 0.645635, 0.478968, 0.336111, 0.211111, 0.100000};

  public static double[]
      coEfft =
      {0.3159, -34.6288, 1.3209, -8.7609, 0., -5.0206};
  ...
  public static boolean isWithinTolerance(double calc, double obsv, double tol) {
    ...
    if (NoOfC13 == 0) {
      if (Math.abs(calc - obsv) > tol)
        return false;
    }
    else {
      double tempError = obsv - calc;
      int isoerr = round(tempError / IsotopeSpace);
      if (isoerr < 0 || NoOfC13 < isoerr)
        return false;
      if (Math.abs(tempError - isoerr * IsotopeSpace) > precursorAccuracy)
        return false;
    }
    return true;
  }
