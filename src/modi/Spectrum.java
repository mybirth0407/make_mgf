...
public class Spectrum extends ArrayList<Peak> implements Comparable<Spectrum> {
  ...
  public double getSumOfSquaredIntensity() {
    return sumOfSquaredIntensity;
  }

  public double getSumOfLastBinIntensity() {
    return sumOfLastBinIntensity;
  }

  public double getSumOfLastBinSquaredIntensity() {
    return sumOfLastBinSquaredIntensity;
  }

  public void setSumOfIntensity(double value) {
    this.sumOfIntensity = value;
  }

  public void setSumOfSquaredIntensity(double value) {
    this.sumOfSquaredIntensity = value;
  }

  public void setSumOfLastBinIntensity(double value) {
    this.sumOfLastBinIntensity = value;
  }

  public void setSumOfLastBinSquaredIntensity(double value) {
    this.sumOfLastBinSquaredIntensity = value;
  }
  ...
  public double getBasePaekIntensity() {
    return bpIntensity;
  }
  ...
  public ArrayList<Peak> getSelectedPeaks() {
    return selectedPeak;
  }
  ...
  public void normalizeIntensityLocally() {
    for (Peak p : this) {
      /** Author: Yedarm Seong
       * Q. getLocalMaxPeak 은 MS/MS 인가
       * A. global normalization 은 전체에서 가장 큰 peak 을 1로
       *    local normalization 은 부분부분 자른 조각 중에서
       *    가장 큰 peak 을 1 로 Normalization 함 */
      /** NormIntensity 는 Peak 의 y 축 값(intensity)에서, 해당 Window 에서
   ...
   * 전달된 인자 center 는 해당 Peak 의 X 축에 해당하는 Mass 값
   * Q. Window 란 MS Sepctrum 의 하나의 Peak 을 뜻하는가?
   * A. Window 는 하나의 Mass 에서 일정 크기만큼 좌우 범위를 뜻함
   */
  public double getLocalMaxPeak(double center) {
    double lMax = 0;
    int bin = 50;    // Window 의 크기
    /**
     * binarySearchforPeaks 함수에 mass 값에서
     * Window 의 크기를 뺀값을 인자로 전달하여 index 값 설정
     */
    /**
     * Author: Yedarm Seong
     * 해당 mass 가 몇 번째 mass 인지
     */
...
//    System.out.println(index + " " + center);
    /**
     * index 를 증가시켜주며
     * 해당 index 의 mass 값이 center + bin 보다 작거나 같으면 계속 실행
     */
    /**
     * Author: Yedarm Seong
     * 해당 index 의 mass 가 window 안에 속한다면
     */
    while (this.get(index).getMass() <= center + bin) {
      /** 해당 index 의 intensity 가 lMax 값 보다 크다면 */
      if (this.get(index).getIntensity() > lMax) {
        /** lMax 값은 해당 index 의 intensity 로 변경 */
        lMax = this.get(index).getIntensity();
      }
      index++;    // index 값 증가
      if (index == this.size())    // 현재 index 가 Window 의 마지막 Peak 일 때
        break;
    }
    return lMax;    // 현재 Window 에서 intensity 가 가장 큰 Peak의 intensity
  }

  /**
   * 전달된 인자 center 는 해당 Peak 의 X 축에 해당하는 Mass 값
   */
  public double forNewNormalization_getLocalMaxPeak(double center) {
    double lMax = 0;
    int bin = 25;    // Window 의 크기
    /**
     * binarySearchforPeaks 함수에 mass 값에서
     * Window 의 크기를 뺀값을 인자로 전달하여 index 값설정
     */
    int index = binarySearchforPeaks(center - bin);
     ...
     */
    while (this.get(index).getMass() > center - bin
        && this.get(index).getMass() <= center + bin) {
      /** 해당 index 의 y 축이 lMax 값 보다 크다면 */
      if (this.get(index).getIntensity() > lMax) {
        /** lMax 값은 해당 index 의 y 축 으로 변경 */
        lMax = this.get(index).getIntensity();
      }
      index++;    // index 값 증가
      if (index == this.size())    // 현재 index 가 Window 의 마지막 Peak 일 때
...
/*---------------------------------------------------------------------------------------------------------------
  ...
  public int binarySearchforPeaks(double left) {    //input : left mass
    int index;
    if (left <= this.get(0).getMass())    //초기값 처리
      index = 0;
    else if (left > this.get(this.size() - 1).getMass())    //끝처리
      index = this.size() - 1;
    else {
  ...
  public double getPeakEvidence(Peak p) {
    ...
    if (p.property == PeakProperty.N_TERM_B_ION_ONLY || p.property == PeakProperty.N_TERM_Y_ION_ONLY)
      return 1;

    if (p.property == PeakProperty.C_TERM_B_ION_ONLY || p.property == PeakProperty.C_TERM_Y_ION_ONLY)
      return 1;

    if (p.getNormIntensity() < (it = getMatchedPeak(mz - 1.0))) {
      return ev;
    }    //Isotope

    ev += getMatchedPeak(mz + 1.0);
    if (p.getNormIntensity() >= (it = getMatchedPeak(mz - 17.0))) {
      ev += it;
    }    //NH3
    if (p.getNormIntensity() >= (it = getMatchedPeak(mz - 18.0))) {
      ev += it;
    }    //H2O

    ev +=
        getMatchedPeak(p.getComplementMass(correctedMW));         //Complementary */
  ...
  public boolean isConfidentPeak(Peak tar, double factor) {
    ...
    if (me > 0 && tar.getMass() - this.get(me - 1).getMass() < 1.5) {
      if (tar.getIntensity() / 5 < this.get(me - 1).getIntensity())
        return false;
    }

    if (me < this.size() - 1 && this.get(me + 1).getMass() - tar.getMass() < 1.5) {
      if (tar.getIntensity() < this.get(me + 1).getIntensity())
        return false;
    }
  ...
  public int compareTo(Spectrum s) {
    if (observedMW > s.observedMW)
      return 1;
    else if (observedMW == s.observedMW)
      return 0;
    else
      return -1;
  }
  ...
  private void setScoreOfSelectedPeaks(ArrayList<Peak> selected, int assumedCS, double tolerance) {
    ...
    for (int node = 0; node < selected.size(); node++) {
      ...
      if (selected.get(node).property == PeakProperty.N_TERM_B_ION_ONLY || selected.get(node).property == PeakProperty.N_TERM_Y_ION_ONLY) {
        selected.get(node).setProbability(1.);
        continue;
      }

      if (selected.get(node).property == PeakProperty.C_TERM_B_ION_ONLY || selected.get(node).property == PeakProperty.C_TERM_Y_ION_ONLY) {
        selected.get(node).setProbability(1.);
        continue;
      }
      ...
      for (int i = node - 1; i > -1; i--) {
        if (selected.get(i).getMass() < targetmz - tolerance)
          break;
      ...
      if (OK != -1) {
        if (okmax > ionIT)
          selected.get(node).setProbability(0.);
        else
      ...
      for (int nth_iso = 1; ; nth_iso++) {
        ...
        for (int i = ISO; i < selected.size(); i++) {
          if (selected.get(i).getMass() > targetmz + Constants.massToleranceForDenovo)
            break;
      ...
      if (NLOSS != -1) {
        ...
        if (NL_H != -1)
          lossScore += selected.get(NL_H).getNormIntensity();
        score += lossScore * 0.5;
