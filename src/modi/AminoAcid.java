...
public class AminoAcid implements Comparable<AminoAcid> {
  ...
  public AminoAcid(char residue, double monoMass, double avgMass, String fullName, int index) {
    ...
    if (AminoAcid.indexSize < index + 1)
      AminoAcid.indexSize = index + 1;
  }
  ...
  public static double getMass(char residue) {
    return getAminoAcid(residue).getMass();
  }
  ...
  public static ArrayList<AminoAcid> getCode(double mass, double massTolerance) {
    ...
    for (int i = 0; i < aaTable.length; i++) {
      ...
      else if (aaTable[i].getMass() < mass + massTolerance && aaTable[i].getMass() > mass - massTolerance)
  ...
  private static AminoAcid[]
      aaTable =
      {new AminoAcid('A', 71.037114, 71.08, "alanine", 0), null, //new AminoAcid('B',  0,          0,      "_invalid_" ),
          //  new AminoAcid('C',  103.00919+57.0215,  103.1+57.0213,  "cysteine",         1),
          new AminoAcid('C', 103.00919, 103.1, "cysteine", 1), new AminoAcid('D', 115.02694, 115.1, "aspartate", 2), new AminoAcid('E', 129.04259, 129.1, "glutamate", 3), new AminoAcid('F', 147.06841, 147.2, "phenylalanine", 4), new AminoAcid('G', 57.021464, 57.05, "glycine", 5), new AminoAcid('H', 137.05891, 137.1, "histidine", 6), new AminoAcid('I', 113.08406, 113.2, "isoleucine", 7), null, //new AminoAcid('J',  0,          0,      "_invalid_"),
          new AminoAcid('K', 128.09496, 128.2, "lysine", 8), new AminoAcid('L', 113.08406, 113.2, "leucine", 9), new AminoAcid('M', 131.04048, 131.2, "methionine", 10), new AminoAcid('N', 114.04293, 114.1, "asparagine", 11), null, //new AminoAcid('O',  0,          0,      "_invalid_"),
          new AminoAcid('P', 97.052764, 97.12, "proline", 12), new AminoAcid('Q', 128.05858, 128.1, "glutamine", 13), new AminoAcid('R', 156.10111, 156.2, "arginine", 14), new AminoAcid('S', 87.032029, 87.08, "serine", 15), new AminoAcid('T', 101.04768, 101.1, "threonine", 16), null, //new AminoAcid('U',  151.00919,  151.1,  "selenocysteine",   20),
          new AminoAcid('V', 99.068414, 99.07, "valine", 17), new AminoAcid('W', 186.07931, 186.2, "tryptophan", 18), null, //new AminoAcid('X',  0,          0,      "_invalid_"),
          new AminoAcid('Y', 163.06333, 163.2, "tyrosine", 19), null, //new AminoAcid('Z',  0,          0,      "_invalid_"),
