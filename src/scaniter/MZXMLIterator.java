  ...
  public MZXMLIterator(String fileName) throws IOException {
    ...
    try {
      ...
      for (int i = 1; i <= maxScanNo; i++) {
        ...
        if (shead.getMsLevel() > 1)
          size++; // skip MS spectrum(select MSMS spectrum only)
      }//*/
  ...
  public ArrayList<MSMScan> getNext() throws IOException {
    ...
    for (int i = scanNo; i <= maxScanNo; i++) {
      ...
      if (scan == null)
        continue;
      ScanHeader shead = scan.getHeader();
      if (shead.getMsLevel() < 2)
        continue; // skip MS spectrum(select MSMS spectrum only)

      double[][] peakList = scan.getMassIntensityList(); // get peak list array

      int
          precursorCharge =
          shead.getPrecursorCharge(); // if charge is not determined, guess charge
      if (precursorCharge < 1) {
        ...
        for (int j = 0; j < peakList[0].length; j++)
          if (peakList[0][j] <= shead.getPrecursorMz())
            intBelow++;
          else
            intAbove++;

        if ((double) intAbove < 0.1 * (double) intBelow)
          precursorCharge = 1;
      }
      ...
      if (precursorCharge < 1) {
        for (int cs = MIN_ASSUMED_CHARGE; cs <= MAX_ASSUMED_CHARGE; cs++) {
          curScan =
              new MSMScan(String.valueOf(scanIndex + 1), -1, shead.getPrecursorMz(), cs);
        ...
        curScan =
            new MSMScan(String.valueOf(scanIndex + 1), -1, shead.getPrecursorMz(), precursorCharge);
        curScan.setOffset(offsetMap.get(i));
        curScan.readPeakList(peakList);
        if (curScan.getSpectrum() != null)
          scanlist.add(curScan);
