...
public class MSMScan {
	...
	public Spectrum getSpectrum(){ return peaklist; }
	...
	public void readPeakList( RandomAccessFile in ) throws IOException {
		...
		if( Constants.rangeForIsotopeIncrement != 0 )
			Constants.NoOfC13 = (int)Math.ceil( neutralMW / Constants.rangeForIsotopeIncrement );					
		if( Constants.PPMTolerance != 0 )
			Constants.precursorAccuracy = Constants.PPMtoDalton(neutralMW, Constants.PPMTolerance);		
		Constants.precursorTolerance = Constants.precursorAccuracy + Constants.NoOfC13*Constants.IsotopeSpace;
		...
		if (Constants.iTRAQSearch) {
			...
			double oneChargeRightTagMass = this.neutralMW - isobaricTag + Constants.Proton;
			double twoChargeRightTagMass = (this.neutralMW - isobaricTag + (Constants.Proton * 2)) / 2;
				...
				if (Math.abs(ionMZ-reporterIon114) < reporterIonTolerance || Math.abs(ionMZ-reporterIon115) < reporterIonTolerance || Math.abs(ionMZ-reporterIon116) < reporterIonTolerance || Math.abs(ionMZ-reporterIon117) < reporterIonTolerance) {
					...
					for (int nth_iso=1; ; nth_iso++) {
						...
						for (int i=ISO; i<rawPL.size(); i++) {
							if (rawPL.get(i).mz > targetmz+isobarictagTolerance) { break; }
		...
		for( RawP rp : rawPL ) {
				...
				if( Math.abs( mass-pmz ) < 5. ) continue;				
		...
		if( spectrum.size() < minPeaksCount || neutralMW < minMW ) peaklist = null; 
